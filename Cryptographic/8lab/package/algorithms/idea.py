...
from package.types import IDEAMathNum
...
class IDEA(CipherCore):
    ...
    def __division_into_subblocks(self, block: str) -> list[IDEAMathNum]:
        return [
            IDEAMathNum.init_from_binary_string(block[i*16:(i+1)*16])
            for i in range(4)
        ]

    def __round(self, subblocks: list[IDEAMathNum], subkeys: list[IDEAMathNum]) -> None:
    ...
    def __output_conversion(self, subblock: list[IDEAMathNum], subkeys: list[IDEAMathNum]) -> None:
    ...
    def __generate_encrypt_subkeys(self, key: int) -> list[list[IDEAMathNum]]:
        base_subkeys = self.__generate_base_subkeys(key)
        return self.__split_on_round_subkeys(base_subkeys)

    def __generate_decrypt_subkeys(self, key: int) -> list[list[IDEAMathNum]]:
        base_subkeys = self.__generate_base_subkeys(key)
    ...
    def __generate_base_subkeys(self, key: int) -> list[IDEAMathNum]:
        bin_password = "{:0128b}".format(key)
        round_subkeys = []

        for _ in range(7):
            for i in range(8):
                round_subkeys.append(IDEAMathNum.init_from_binary_string(
                    bin_password[i*16: (i+1)*16]))
            bin_password = bin_password[25:] + bin_password[:25]

        return round_subkeys[:-4]

    def __split_on_round_subkeys(self, subkeys: list[IDEAMathNum]) -> list[list[IDEAMathNum]]:
        return [subkeys[i*6:(i+1)*6] for i in range(9)]

    def __change_order_list(self, subkeys: list[list[IDEAMathNum]]) -> list[list[IDEAMathNum]]:
        subkeys = subkeys[::-1]
        decrypt_subkeys = [
            [
                *self.__inverse_subkeys(subkeys[i][:4], i),
                *subkeys[i+1][4:]
            ]
    ...
    def __inverse_subkeys(self, subkeys: list[IDEAMathNum], i: int) -> list[IDEAMathNum]:
        if i > 0 and i < 8:
